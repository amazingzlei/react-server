...
import com.fh.project.service.ChatServiceImpl;
		...
		if(obj.containsKey("role")){
			...
			if (tos.contains(",")) {
				...
				for (String str : split) {
					send(info,role,str,socketId);
				}
			}else {
				to = (String) obj.get("role");
				send(info,role,to,socketId);
			}
		}
		ChatServiceImpl chatService=(ChatServiceImpl) ContextLoader.getCurrentWebApplicationContext().getBean("chatService");
		chatService.addChat(from, to, info);
	}
